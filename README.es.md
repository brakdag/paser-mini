...
- `global_search(query)`, `glob_search(pattern)`, `global_replace(path, search_text, replace_text, extensiones)`: Bóqueda y reemplazo masivo.

### 🔢 Navegación de Código

- `list_symbols(file_path)`: Lista todas las clases y funciones definidas en un archivo.
- `get_definition(symbol_name, file_path)`: Localiza la línea y columna donde se define un sómbolo.
- `get_references(symbol_name, file_path)`: Busca todas las referencias a un sómbolo en el archivo.
