...
1. Iniciar instancia.
2. Ejecutar `pull_memory()` (Efecto Espejo).
...
2. Monitorear el uso de tokens mediante `get_token_count()`.
...
2. Ejecutar `push_memory(scope='fractal', ...)` para guardar puntos clave.

### Fase 4: El Puente (95% $\rightarrow$ 100%)
1. Al llegar al 95%, el agente debe generar un **Bloque Puente (Bridge Block)**.
2. Guardar el puente mediante `push_memory` con el teaser `BRIDGE: [Resumen de Sesión]`.
...
- **Guardado**: El agente hace un `push_memory` con el resumen del archivo y la ruta del mismo.
- **Recuperación**: El agente usa `pull_memory` $\rightarrow$ encuentra la ruta del archivo $\rightarrow$ usa `read_file` para leer el contenido actualizado del disco.