...
- **Technical Solution**: 
    - Add `timeout=30` to all `subprocess.run` calls in `search_files_pattern` and `search_text_global`.
...
### 2.3. Non-Exact Repetition Loops (Behavioral Loops)
- **File**: `paser/core/chat_manager.py`
- **Observed Behavior**: `RepetitionDetector` only catches exact n-gram repetitions. Minor variations in tool arguments bypass this detection.
- **Risk**: The agent may enter a "retry loop" where it calls the same tool with slight variations, consuming API quota without progressing.
- **Technical Solution**:
    - Implement a `ToolAttemptTracker` within the `execute` loop.
    - Track the number of consecutive failures for the same tool name.
    - If a specific tool fails more than 3 times consecutively in a single turn sequence, inject a system warning or force a break to prevent API exhaustion.
