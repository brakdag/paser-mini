...
class MementoManager:
    ...
    async def push_memory(self, role: str, scope: str, value: str, key: Optional[str] = None, pointers: Optional[List[int]] = None) -> str:
        ...
        # 4. Store Node
        node_id = await self.db.push_node(role, node_type, value, teaser, is_vital)
        ...
        for target_id in all_links:
            await self.db.increment_weight(target_id)
            await self.db.add_edge(node_id, target_id, 'associative')
    ...
    async def pull_memory(self, scope: Optional[str] = None, key: Optional[str] = None, direction: Optional[str] = None) -> str:
        ...
        if key:
            ...
            # Referenced by
            refs = await self.db.get_referenced_by(node['id'])
            if refs:
    ...
    async def get_latest_bridge(self) -> Optional[Dict[str, Any]]:
        ...
        return await self.db.get_latest_bridge()
