...
export const editIssue = async ({
  ...
  try {
    ...
    await client.patch(`/repos/${targetRepo}/issues/${issueNumber}`, data, { headers });
...
export const closeIssue = async ({ issueNumber, repo = '' }) => {
  try {
    const targetRepo = repo ? resolveRepo(repo) : (await getCurrentRepo());
    const headers = await getHeaders();
    await client.patch(`/repos/${targetRepo}/issues/${issueNumber}`, { state: 'closed' }, { headers });
...
export const postComment = async ({ issueNumber, body, repo = '' }) => {
  try {
    const targetRepo = repo ? resolveRepo(repo) : (await getCurrentRepo());
    const headers = await getHeaders();
    await client.post(`/repos/${targetRepo}/issues/${issueNumber}/comments`, { body }, { headers });
...
export const addLabel = async ({ issueNumber, label, repo = '' }) => {
  try {
    const targetRepo = repo ? resolveRepo(repo) : (await getCurrentRepo());
    const headers = await getHeaders();
    await client.post(`/repos/${targetRepo}/issues/${issueNumber}/labels`, { labels: [label] }, { headers });
...
export const removeLabel = async ({ issueNumber, label, repo = '' }) => {
  try {
    const targetRepo = repo ? resolveRepo(repo) : (await getCurrentRepo());
    const headers = await getHeaders();
    await client.delete(`/repos/${targetRepo}/issues/${issueNumber}/labels/${label}`, { headers });
...
export const getIssueComments = async ({ issueNumber, repo = '' }) => {
  try {
    const targetRepo = repo ? resolveRepo(repo) : (await getCurrentRepo());
    const headers = await getHeaders();
    const response = await client.get(`/repos/${targetRepo}/issues/${issueNumber}/comments`, { headers });
    return response.data;
  } catch (e) {
    return `ERR: ${e.message}`;
  }
