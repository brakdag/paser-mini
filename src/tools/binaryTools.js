...
async function extractBinary(filePath, start, end, length, outputFile) {
  ...
  if (readLength <= 0) throw new Error('Invalid extraction range');
...
async function searchBinary(filePath, pattern) {
  ...
  while (bytesRead < fileSize) {
    const { bytesRead: read } = await handle.read(buffer, 0, bufferSize, bytesRead);
    if (read === 0) break;

    let index = buffer.indexOf(searchBuf);
    while (index !== -1) {
      offsets.push(bytesRead + index);
      index = buffer.indexOf(searchBuf, index + 1);
    }

    bytesRead += read - (searchBuf.length - 1);
