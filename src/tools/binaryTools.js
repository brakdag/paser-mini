...
export class BinaryTools {
  ...
  async handleHexCommand(args) {
    ...
    try {
      switch (action) {
        case "inspect":
          return await this.#inspectBinary(filePath, offset, length);
        case "extract":
          return await this.#extractBinary(filePath, offset, end, length, outputFile);
        case "search":
          return await this.#searchBinary(filePath, pattern);
        case "detect":
          return await this.#detectBinary(filePath);
        case "convert":
          return this.#convertBinary(hexString, type, endianness);
        default:
