export class InterfaceCommands {
  ...
  static async handleNick(chatManager, ui, newNick) {
    ...
    ui.displaySystemMessage(`${oldNick} is now known as ${newNick}`);
  ...
  static async handleAction(chatManager, ui, actionText) {
    if (ui.renderingMode === 'FOUNTAIN') {
      ...
      const formatted = ui._renderFountain('system', `* ACTION: ${actionText} *`);
  ...
  static async handleJoin(chatManager, ui, channel) {
    ...
    if (ui.renderingMode === 'FOUNTAIN') {
      ...
      const content = ui.renderingMode === 'FOUNTAIN' ? ui._renderFountain('system', systemMsg) : systemMsg;
