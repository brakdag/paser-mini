...
class AutoCorrector:
    ...
    def fix_json(content: str) -> str:
        ...
        for open_c, close_c in [('{', '}'), ('[', ']')]:
            ...
            if diff > 0: content += close_c * diff
            elif diff < 0: content = open_c * abs(diff) + content
        return content

class SmartToolParser:
    TOOL_PATTERN = re.compile(r'<(?:TOOL_CALL|tool_call)\s*>(.*?)(?:</(?:TOOL_CALL|tool_call)>|$)', re.IGNORECASE | re.DOTALL)
    ...
    def extract_tool_calls(self, text: str) -> list[tuple[Optional[dict[str, Any]], str, Optional[str]]]:
        return [(self.parse_call(m.group(1).strip())[0], m.group(1).strip(), self.parse_call(m.group(1).strip())[1]) for m in self.TOOL_PATTERN.finditer(text)]
