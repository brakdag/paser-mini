...
export class CommandHandler {
  ...
  async handle(userInput) {
    ...
    if (inputStripped.startsWith('/nick ')) {
      ...
      this.chatManager.assistant.updateNicknames(
        newNick,
        this.chatManager.ui.agentNickname
      );
    ...
    if (inputStripped === '/config') {
      return ConfigCommands.handleConfig(this.chatManager, this.ui);
    }
    if (inputStripped === '/models_check') {
      return await ModelCommands.handleModelsCheck(
        this.chatManager,
        this.ui,
        inputStripped.split(/\s+/)
      );
    }
    if (inputStripped.startsWith('/models')) {
      return await ModelCommands.handleModels(
        this.chatManager,
        this.ui,
        inputStripped.split(/\s+/)
      );
    }
    if (inputStripped.startsWith('/fav')) {
      return await FavoriteCommands.handleFav(
        this.chatManager,
        this.ui,
        inputStripped.split(/\s+/)
      );
