...
async def main():
    ...
    parser.add_argument("-v", "--version", action="version", version="src 0.1.0")
    parser.add_argument(
        "-ut", "--unit_tests", action="store_true", help="Run unit tests"
    )
    parser.add_argument(
        "-si", "--system_instruction", help="Custom system instructions"
    )
    parser.add_argument(
        "-isi",
        "--inject_system_instruction",
        help="Inject instruction at the start of system prompt",
    )
    parser.add_argument(
        "-fsi",
        "--file_system_instruction",
        help="Path to file for system instruction injection",
    )
    parser.add_argument(
        "-m", "--message", help="Initial message to send (one-shot mode)"
    )
    ...
    parser.add_argument(
        ...
        "--instance-mode",
        action="store_true",
        help="Run in instance mode (read-only config and no recursion)",
    )
    parser.add_argument(
        "--run-instance-timeout",
        "-rito",
        type=int,
        help="Timeout for run_instance tool in seconds",
    )
    parser.add_argument(
    ...
    try:
        ui = TerminalUI(
            no_spinner=args.instance_mode, force_terminal=not args.instance_mode
        )
    ...
    base_instr = (
        args.system_instruction if args.system_instruction else SYSTEM_INSTRUCTION
    )
    ...
    chat_manager = ChatManager(
        assistant, AVAILABLE_TOOLS, sys_instr, ui, instance_mode=args.instance_mode
    )
